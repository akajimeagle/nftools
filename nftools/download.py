...
async def get_owners(session, nft, rpc, *, limiter):
    async with limiter:
...
async def get_nfts(collection_creator, rpc, *, refresh) -> List[MetaplexNFT]:
    ...
    async with aiohttp.ClientSession() as session:
        tasks = []
        for nft in nfts:
            tasks.append(get_owners(session, nft, rpc, limiter=limiter))
...
async def main():
    nfts = await dl_owners(collection_creator='8ArpT5ZAGCSAMMAwSw6daH2nHk4DHBmgEvVmPmurTHJA', rpc=RPC, refresh=False,
                           fmt='csv', output=None)
